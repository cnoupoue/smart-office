...
def get_coordinates(gpgga_data):
    ...
    # Convert raw latitude to decimal degrees
    latitude = float(raw_latitude[:2]) + float(raw_latitude[2:]) / 60
    if latitude_direction == 'S':
        latitude = -latitude

    # Convert raw longitude to decimal degrees
    longitude = float(raw_longitude[:3]) + float(raw_longitude[3:]) / 60
