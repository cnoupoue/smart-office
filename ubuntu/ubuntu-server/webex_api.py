...
messages_url = f'https://webexapis.com/v1/messages?roomId={env.WEBEX_COMMANDS_ROOM_ID}'
headers = {
...
def send(message):
    ...
    message_data = {
        'roomId': env.WEBEX_COMMANDS_ROOM_ID,
        'text': '[BOT]: ' + message
    }
    response = requests.post(message_url, headers=headers, data=json.dumps(message_data))
    print(f'Message sent: {response.status_code} {message}')
...
def wait_for_command():
    while True:
        ...
        time.sleep(env.WEBEX_MESSAGE_READ_DELAY)
